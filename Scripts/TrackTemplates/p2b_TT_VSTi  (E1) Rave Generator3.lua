...
if sel_track_count > 0 then
    ...
    -- Расчет длины ровно в 1/4 такта (1 четверть)
    local end_pos = reaper.TimeMap2_beatsToTime(0, 0.5, 0) + start_pos
    
    -- E1 = MIDI pitch 28 (при C4 = 60)
    local pitch = 28 
    local vel = 127
    local channel = 0 -- 1-й MIDI канал
    ...
    for i = 0, sel_track_count - 1 do
        ...
        if item then
            ...
            if take then
                ...
                -- Нота E1 во всю длину (1/4 такта)
...
reaper.Undo_EndBlock("Import " .. template_name .. " with 1/4 Looped MIDI", -1)
